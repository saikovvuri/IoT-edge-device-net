namespace SampleModule {
    ...
    class Program {
        ...
        static Task<MethodResponse> PingMethod (MethodRequest methodRequest, object userContext) {
            ...
            PingRequest request = JsonConvert.DeserializeObject<PingRequest>( methodRequest.DataAsJson); 
            ...
            try {
                pinger = new Ping ();
                PingReply reply = pinger.Send (request.SendAddress);
                pingable = reply.Status == IPStatus.Success;
                 if(pingable)
                     Console.WriteLine($"Ping Test to {request.SendAddress} was successful");
                else 
                     Console.WriteLine($"Ping Test to {request.SendAddress} was unsuccessful");
            } catch (PingException ex) {
                // Discard PingExceptions and return false;
                pingable = false;
                Console.WriteLine($"\t > Ping exception message: {ex.InnerException.Message}");
                response = new MethodResponse((int) System.Net.HttpStatusCode.InternalServerError);
            } finally {
                if (pinger != null) {
                    pinger.Dispose ();
                }
            ...
            response = new MethodResponse ((int) System.Net.HttpStatusCode.OK);
            return Task.FromResult (response);
        }
        ...
        {
            [JsonConstructor]
            public PingRequest (string sendAddress)
            {
